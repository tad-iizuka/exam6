...
Server::Server(char *po, char *pa) : _po(atoi(po)), _pa(pa), _svFd(-1), _epFd(-1) {};
Server::~Server()  {
	if (_epFd != -1)
		close(_epFd);
	for (std::map<int, std::string>::iterator it = _cs.begin(); it != _cs.end(); ++it) {
		close(it->first);
	}
	if (_svFd != -1)
		close(_svFd);
...
int main(int argc, char *argv[]) {
	if (argc != 3) {
		std::cerr << "Usage: " << argv[0] << " <port> <db path>" << std::endl;
		return 1;
	}
